...
function Home({ onSaveGameData }) {
  ...
  const { addUser: { isAdded, error: addUserError } } = useSelector((state) => state);
  const {
  ...
  const handleExistingGame = (e) => {
   ...
   const getWordCount = () => {
     if (remainingCount === "") {
       return `Kindly Enter Game Words`;
     }
     if (remainingCount === 1) {
       return `You have ${remainingCount} word left`;
     }
     if (remainingCount <= 0) {
       return `No spaces left on board`;
     }
     return `You have ${remainingCount} words left`;
   };
          ...
          <div className={container_content_inner}>
            ...
            {!existingGame ? (
              ...
              <div className={formInput}>
                ...
                <div className={btns}>
                  <button id="create--game" className={btns_more} type="submit">
                    {" "}
                    Start Game{" "}
                  </button>
                  <p className={or}>OR</p>
